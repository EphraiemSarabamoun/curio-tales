...
async def text_to_speech(req: TTSRequest):
    """Proxy TTS request to ElevenLabs API. Returns audio/mpeg stream."""
    ...
    if response.status_code != 200:
        logger.error("ElevenLabs TTS failed: %s %s", response.status_code, response.text[:200])
        raise HTTPException(502, f"TTS generation failed ({response.status_code})")
